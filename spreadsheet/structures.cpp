...
std::string_view FormulaError::ToString() const {
    switch (category_) {
        ...
        default:
            return "#CAN'T IDENTIFY ERROR!"sv;
    }
}

FormulaError::FormulaError(std::string_view string_category) {
    if (string_category == "#REF!"sv) {
        category_ = Category::Ref;
    } else if (string_category == "#VALUE!"sv){
        category_ = Category::Value;
    } else if (string_category == "#ARITHM!"sv){
        category_ = Category::Div0;
    }
}
